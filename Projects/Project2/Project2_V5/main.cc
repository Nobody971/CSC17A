...
const short COLS=5;//for memory array
//Function Prototypes
...
void turn(Player[],short,Deck*,char[],short,short,const short);
//determines what card p2 will ask for
void pRqst(Player&,char[]);
//tests if total number of points = 26
bool isEnd(Player[],short);
//determines winner and ranks player
bool order(Player[],short,short[],short[]);
//begin main function
int main() {
    ...
    char option1,//Y or N for playing again
         ...
         name[User.getSize()],//computer players name
    ...
    short noPs,//number of players
          asked=1,//player being asked for a card, default is player 2 (a.k.a. index 1)
          *asker,//dynamic array for remembering who asked for what
          *scores,//dynamic array for sorting player scores
          *pNums;//dynamic array parallel to scores[], holds player nos for ranking performance
    Player *P;//dynamic array of players
        ...
        do //do while loop for repeating cycle of turns until all cards are gone       
        {
            ...
            do//do while loop so Player 1 gets another turn if player 2 has requested card
            {
                ...
                if (match)
                {
                    ...
                    out=isOut(P[asked]);
                    if(out){restock(P[asked],Deck1,NCARDS);}
                    //call dspPnts to display all players' points
                    dspPnts(P,noPs,User.getSize());
            ...
            for(short i=1;i<noPs;i++)
                turn(P,i,Deck1,request,noPs,User.getSize(),NCARDS);
...
void dspPnts(Player P[],short noPs,short width)
{
    for(short i=0;i<noPs;i++)
    {
        ...
        cout<<right<<" has "<<P[i].getPnts()<<((P[i].getPnts()==1)?" point.":" points.")<<endl;
    }
}
void turn(Player P[],short index,Deck *Deck1,char request[],short noPs,short width,const short NCARDS)
{
    bool match,out;
    short asked;
    ...
    if(strcmp(request,"bypass")!=0)
    {
        ...
        do
        {
            ...
            if (match)
            {
                ...
                out=isOut(P[asked-1]);
                if(out){restock(P[asked-1],Deck1,NCARDS);}
                out=isOut(P[index]);
                if(out){restock(P[index],Deck1,NCARDS);}
...
bool order(Player P[],short noPs,short scores[],short pNums[])
{
