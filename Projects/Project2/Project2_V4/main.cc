...
void dspPnts(Player[],short);//displays all players points
...
//tests for players who are out of cards and deals 3 to them if out
void restock(Player&,Deck*,const short);
...
int main() {
    ...
    Deck *Deck1=nullptr;
    Profile User;
    short noPs=2;//number of players
    Player P[2];//array of 2 players
    ...
    do//do while loop so player can play game as many times as desired
    {
        ...
        do //do while loop for repeating cycle of turns until all cards are gone       
        {
            ...
            do//do while loop so Player 1 gets another turn if player 2 has requested card
            {
                ...
                if (match)
                {
                    ...
                    //returns true if so
                    out=isOut(P[0]);
                    if(out){restock(P[0],Deck1,NCARDS);}
                    out=isOut(P[1]);
                    if(out){restock(P[1],Deck1,NCARDS);}
                    //call dspPnts to display all players' points
                    dspPnts(P,noPs);
                    ...
                    if(request=="x")cout<<"You bypass your turn"<<endl;
                ...
                do
                {
                    ...
                    if (match)
                    {
                        ...
                        if(strcmp(request,"bypass")!=0)
                            ...
                            cout<<"Player two is out of cards and bypasses turn."<<endl;
            ...
            else cout<<"Player 2 is out of cards and bypasses turn."<<endl;
        ...
        else if(User.getLctn()==-1)
        {
            ...
            cin.get(option2);
            if(option2=='y'||option2=='Y')User.save(User);
        }
...
{   //if hand array contents at index 0 = contents at index 1, 
    ...
    {
        cout<<"Player "<<P.getNo()<<" has two "<<P.getFace(0)<<((strcmp(P.getFace(0),"Six"))==0?"es! ":"s! ")<<endl;
        P.setFace(0," ");
        P.setSuit(0," ");
        P.setFace(1," ");
        P.setSuit(1," ");
    ...
    {
        cout<<"Player "<<P.getNo()<<" has two "<<P.getFace(1)<<((strcmp(P.getFace(1),"Six"))==0?"es! ":"s! ")<<endl;
        P.setFace(1," ");
        P.setSuit(1," ");
        P.setFace(2," ");
        P.setSuit(2," ");
    ...
    {
        cout<<"Player "<<P.getNo()<<" has two "<<P.getFace(2)<<((strcmp(P.getFace(2),"Six"))==0?"es! ":"s! ")<<endl;
        P.setFace(2," ");
        P.setSuit(2," ");
        P.setFace(3," ");
        P.setSuit(3," ");
    ...
    {
        cout<<"Player "<<P.getNo()<<" has two "<<P.getFace(3)<<((strcmp(P.getFace(3),"Six"))==0?"es! ":"s! ")<<endl;
        P.setFace(3," ");
        P.setSuit(3," ");
        P.setFace(4," ");
        P.setSuit(4," ");
    ...
    {
        cout<<"Player "<<P.getNo()<<" has two "<<P.getFace(4)<<((strcmp(P.getFace(4),"Six"))==0?"es! ":"s! ")<<endl;
        P.setFace(4," ");
        P.setSuit(4," ");
        P.setFace(5," ");
        P.setSuit(5," ");
    ...
    {
        cout<<"Player "<<P.getNo()<<" has two "<<P.getFace(5)<<((strcmp(P.getFace(5),"Six"))==0?"es! ":"s! ")<<endl;
        P.setFace(5," ");
        P.setSuit(5," ");
        P.setFace(6," ");
        P.setSuit(6," ");
...
void dspPnts(Player P[],short noPs)
{
    for(short i=0;i<noPs;i++)
            cout<<"Player "<<P[i].getNo()<<" has "<<P[i].getPnts()
                <<((P[i].getPnts()==1)?" point.":" points.")<<endl;
}
void turn(short p)
{
    cout<<"***************************************************"<<endl;
...
void restock(Player &P,Deck *Deck1,const short NCARDS)
{   //display message informing player
    if(P.getNo()==1){cout<<"You ran of of cards! Draw three from the deck."<<endl;}
    else{cout<<"Player "<<P.getNo()<<" runs out of cards and draws three from the deck."<<endl;}
